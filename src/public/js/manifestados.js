    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var resultados = datos.filter(function (item) {
                ...
                var fecha_fin_timestamp = fecha_fin ? new Date(fecha_fin).getTime() : Number.MAX_SAFE_INTEGER;
                ...
                return (nombre === '' || item_nombre.includes(nombre.normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) &&
                    ...
                    (item_fecha_inscripcion >= fecha_inicio_timestamp && item_fecha_inscripcion <= fecha_fin_timestamp);
            });
            ...
            $(document).ready(function () {
                var table = $('#manifestados').DataTable({
                    ...
                    columns: [
                        ...
                        { "data": "id_contribuyente", "className": "fondo" },
                        { "data": "nombre" },
                        ...
                        { "data": "perfil", "className": 'ocultarColum' },
                        {
                            "data": "fierro",
                        ...
                        }, { "data": "libro", "className": 'ocultarColum' }, { "data": "foja", "className": 'ocultarColum' },
                        { "data": "numero_registro", "className": 'ocultarColum' }, { "data": "estado" }, { "data": "iniciales", "className": 'ocultarColum' },
                        { "data": "folio", "className": 'ocultarColum' }, { "data": "concepto", "className": 'ocultarColum' },
                        { "data": "fecha", "className": 'ocultarColum' }
                        ,
                        {
                            "data": "folio_finanzas",
                        ...
                        },
                        {
                            'defaultContent':
                                ...
                                '<button class="btn btn-success btn-sm perfil"><i class="fa-solid fa-user"> </i></button></div> ' +
                                ...
                                '<div style="margin: 5px 0px 0px 15px; "> <button class="btn btn-success btn-sm certficacion"><i class="fa-solid fa-square-check"></i></button> </div> '

                        },
                    ],
                    columnDefs: [
                        {
                            targets: 7, // Índice de la columna "fecha" (empezando desde 0)
                        ...
                        {
                            targets: 17, // Índice de la columna "fecha" (empezando desde 0)
                            render: function (data) {
                                if (data) {
                ...
                $(document).on("click", ".perfil", function () {
                    ...
                    const id_contribuyente = $("#id_contribuyente").val();
                    ...
                    $(document).ready(function () {
                        ...
                        var tabla = $('#id_pagos').DataTable({
                            ...
                            columnDefs: [
                                {
                                    targets: 3, // Índice de la columna "fecha" (empezando desde 0)
                                    render: function (data) {
                                        if (data) {
                                            var fecha = moment(data);
                                            if (fecha.isValid()) {
                ...
                $(document).on("click", ".certficacion", function () {
                    ...
                    const fechaOriginal = $(`#fecha_inscripcion_cer`).text()
                    ...
                    const fechaFormateada = new Date(anio, mes, dia).toLocaleDateString('es-ES', opcionesFecha);

                    document.getElementById('formateada').innerHTML = `${fechaFormateada}`;
                    ...
                    let mes1 = new Intl.DateTimeFormat('es-ES', opciones).format(fecha);
                    // Obtener el año
                    let año = fecha.getFullYear();
                    // Unir los elementos de la fecha en una cadena con el formato deseado

                    if (dia1 == 1) {
                        let fechaFormateada1 = `a ${dia1} día del mes de ${mes1} del año ${año}.`;
                        document.getElementById('diactual').innerHTML = `${fechaFormateada1}`;
                    } else {
                        let fechaFormateada1 = `a los ${dia1} días del mes de ${mes1} del año ${año}.`;
                        document.getElementById('diactual').innerHTML = `${fechaFormateada1}`;
                    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const $boton = document.querySelector("#manifestar");
    $boton.addEventListener("click", () => {
        const $elementoParaConvertir = document.querySelector("#manifestante"); // <-- Aquí puedes elegir cualquier elemento del DOM
...
document.addEventListener("DOMContentLoaded", () => {
    // Escuchamos el click del botón
    const $boton = document.querySelector("#historial");
    $boton.addEventListener("click", () => {
        var div = document.getElementById("salto");
        div.classList.add("topHistorial");
        const $elementoParaConvertir = document.querySelector("#perfil"); // <-- Aquí puedes elegir cualquier elemento del DOM

        html2pdf()

            .set({
                margin: [0.5, 0.2, 0.2, 0.2], //top, left, button, right
...
document.addEventListener("DOMContentLoaded", () => {
    // Escuchamos el click del botón
    const $boton = document.querySelector("#cancelarCedu");
    $boton.addEventListener("click", () => {
        
        const $elementoParaConvertir = document.querySelector("#cancelar"); // <-- Aquí puedes elegir cualquier elemento del DOM
        html2pdf()
            .set({
                margin: 0.2,
...
document.addEventListener("DOMContentLoaded", () => {
    // Escuchamos el click del botón
    const $boton = document.querySelector("#certificacion");
    $boton.addEventListener("click", () => {
        var div = document.getElementById("addinput");
        div.classList.add("hidden");
        const $elementoParaConvertir = document.querySelector("#cer_id"); // <-- Aquí puedes elegir cualquier elemento del DOM
        html2pdf()
            .set({
                margin: [0, 0.5, 0, 0.5], //top, left, button, right
