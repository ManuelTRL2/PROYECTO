  ...
  $.ajax({
    ...
    success: function success(response) {
      ...
      var resultados = datos.filter(function (item) {
        ...
        var item_fierro = typeof item.fierro === 'string' ? item.fierro.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase() : ''; // Compara la búsqueda con los campos

        return (nombre === '' || item_nombre.includes(nombre.normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) && (apellido_paterno === '' || item_apellido_paterno.includes(apellido_paterno.normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) && (apellido_materno === '' || item_apellido_materno.includes(apellido_materno.normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) && (clave_ine === '' || item_clave_ine.includes(clave_ine.normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) && (fierro === '' || item_fierro.includes(fierro.normalize('NFD').replace(/[\u0300-\u036f]/g, ''))) && ($('#fecha_inicio').val() === '' || new Date(item.fecha_inscripcion) >= new Date($('#fecha_inicio').val())) && ($('#fecha_fin').val() === '' || new Date(item.fecha_inscripcion) <= new Date($('#fecha_fin').val()));
